...
q3 (Tape xs y zs)   | y == 'b' = 'b'                                          --start character not same as end character, reject the palindrome
                    | y == ' ' = 'a'                                          --no characters left, accept the palindrome
                    | y == 'a' = q5 (moveLeft (write (Tape xs y zs) ' '))     --start character the same as end character, transition to q5

q4 (Tape xs y zs)   | y == 'a' = 'b'                                          --start character not same as end character, reject the palindrome
                    | y == ' ' = 'a'                                          --no characters left, accept the palindrome
...
turingPalindrome (xs)   | length (xs) > 4               = 'b'                                                           --reject any string with length greater tahn 4
                        | (xs) == ""                    = 'a'                                                           --accept the empty string
                        | inTMAlphabet (xs) == False    = 'b'                                                           --reject any strings not from the alphabet {'a','b'}
